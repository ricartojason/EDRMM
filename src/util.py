...
def ddi_rate_score(record, path='/home/liufeiyan/T630-BACKUP/baseline/DrugRec/data/mimic-iii/output/ddi_A_weighted_iii.pkl'):
    # ddi rate
...
def ehr_rate_score(record, path='/home/liufeiyan/T630-BACKUP/baseline/DrugRec/data/mimic-iii/output/ehr_A_iii.pkl'):
        ...
        for adm in patient:
            ...
            for i, med_i in enumerate(med_code_set):
                for j, med_j in enumerate(med_code_set):
                    ...
                    '''if ddi_A[med_i, med_j] == 1 or ddi_A[med_j, med_i] == 1:
                        dd_cnt += 1'''
                    if ehr_A[med_i, med_j] > 0 or ehr_A[med_j, med_i] >0:
                        dd_cnt += 1
    if all_cnt == 0:
        return 0
    return dd_cnt / all_cnt
...
def buildPrjSmiles(molecule, med_voc, device="cpu:0"):
    ...
    for index, ndc in med_voc.items():
        ...
        for smiles in smilesList:
            mol = Chem.MolFromSmiles(smiles) # 验证SMILES的有效性
            if mol is not None:
                smiles_all.append(smiles)  # 收集有效的SMILES
                counter += 1
            else:
                print('[SMILES]', smiles)
                print('[Error] Invalid smiles')
        average_index.append(counter) # 记录每个药物的有效SMILES数量

        """Transform the above each data of numpy
        to pytorch tensor on a device (i.e., CPU or GPU).
        """
    # transform into projection matrix
    n_col = sum(average_index) # 所有有效SMILES的总数
    n_row = len(average_index) # 药物的数量
